...
describe('TaxonomyController', () => {
  ...
  it('returns all taxonomy dimensions with value and label pairs', () => {
    ...
    expect(result.moatPatterns).toHaveLength(Object.keys(MOAT_PATTERN_LABELS).length);
    ...
    for (const option of result.moatPatterns) {
      expect(option).toEqual({
        value: option.value,
        label: MOAT_PATTERN_LABELS[option.value as keyof typeof MOAT_PATTERN_LABELS],
      });
    }
  });
});
