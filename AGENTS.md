...
Current reference-only files: `01-ideas/task-pipeline-and-research-tracker-plan.md`, `02-specs/mini-research-tracker-idea.md`.
...
2. Refuse/no-op if the frontmatter has `pipeline_role: reference` (see "Reference-only files" above) — explain it's reference material, not a task. Also refuse/no-op if the file is already in `04-shipped/` — a file in `03-ready/` only ever leaves via the manual post-implementation step above, never via this procedure.
3. Determine the pending transition from the file's current folder: in `01-ideas/` it's "ideas → specs"; in `02-specs/` it's "specs → ready".
