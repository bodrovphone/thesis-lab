...
**These are not part of the `01-ideas` → `02-specs` → `03-ready` → `04-shipped` pipeline.** No `stage`/`domain` frontmatter, no sign-off gate, no `advance-task` procedure applies here — each file is a self-contained, one-shot browser task. Point an agent at exactly one file per session.
